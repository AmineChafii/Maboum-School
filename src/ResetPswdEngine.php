...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt_check_email->rowCount() === 0) {
        $_SESSION['error-email'] = "L'adresse e-mail n'existe pas !";
        header("Location: ../view/mot_de_passe_oublie.php");
        exit();
    }else{
        $email = $_POST['email'];
        ...
        try{
            ...
            $verification_code_reset = substr(number_format(time() * rand(), 0, '', ''), 0, 6);

            $mail->Subject = '=?UTF-8?B?' . base64_encode('Récupération de votre compte') . '?=';
            ...
            $_SESSION['verification_code_reset'] = $verification_code_reset;
            $_SESSION['verification_code_created_at'] = time();
            $_SESSION['email'] = $email;

            header("Location:../view/VerifCodeReset.php");
            exit();
